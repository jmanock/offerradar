...
type SourceFilter = "all" | "recent" | "official" | "affiliate" | "research" | "needs_review";
...
export function OfferHubExplorer({ offers, categories }: { offers: Offer[]; categories: CategoryInfo[] }) {
  ...
  const visible = offers.filter((offer) => {
    ...
    return (!search || text.includes(search.toLowerCase())) && (category === "all" || offer.category === category) && (state === "all" || offer.stateRestrictions?.includes(state)) && (source === "all" || (source === "recent" ? recent : source === "needs_review" ? offer.verificationStatus === "needs_review" : sourceKind === source));
  });
