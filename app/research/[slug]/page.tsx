...
export default async function ResearchArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return <article>
    ...
    <div className="mx-auto grid max-w-5xl gap-8 px-4 py-12 sm:px-6 lg:grid-cols-[1fr_260px]">
      <div className="grid gap-6"><aside className="rounded-3xl border border-teal-200 bg-teal-50 p-6"><p className="text-xs font-extrabold uppercase tracking-wide text-teal-800">Key takeaway</p><p className="mt-2 text-lg font-bold leading-8 text-slate-900">{article.takeaway}</p></aside>{article.sections.map((section, index) => <section key={section.title} className="premium-card rounded-3xl p-6"><p className="text-xs font-black text-teal-700">0{index + 1}</p><h2 className="mt-2 text-2xl font-black text-slate-950">{section.title}</h2><p className="mt-3 leading-7 text-slate-600">{section.body}</p><ul className="mt-5 grid gap-2">{section.points.map((point) => <li key={point} className="flex gap-3 rounded-2xl bg-slate-50 p-3 text-sm font-semibold text-slate-700"><span className="text-teal-600">✓</span>{point}</li>)}</ul></section>)}{article.affiliatePlacements?.length ? <section className="rounded-3xl border border-blue-200 bg-blue-50 p-6"><p className="text-xs font-extrabold uppercase tracking-wide text-blue-800">Contextual external tools</p><h2 className="mt-2 text-2xl font-black text-slate-950">Continue your verification</h2><div className="mt-5 grid gap-4 md:grid-cols-2">{article.affiliatePlacements.map((placement) => <article key={placement.affiliateId} className="rounded-2xl border border-blue-100 bg-white p-5"><h3 className="text-lg font-black text-slate-950">{placement.heading}</h3><p className="mt-2 text-sm leading-6 text-slate-700">{placement.body}</p><div className="mt-4"><AffiliateLink affiliateId={placement.affiliateId} placementId={placement.placementId} pagePath={`/research/${article.slug}`} linkText={placement.linkText} /></div></article>)}</div><div className="mt-5"><AffiliateDisclosure compact /></div></section> : null}{article.showTravelNetwork ? <NetworkOpportunityModule sourcePage={`/research/${article.slug}`} /> : null}</div>
