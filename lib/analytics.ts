...
function firstPartyEvent(name:string,params:AnalyticsParams){
  ...
  const payload=JSON.stringify({name,source,page:String(params.source_page||params.page||window.location.pathname).slice(0,160),return_state:seen?"returning":"new",dimension:String(params.comparison_providers||params.provider||params.network_destination||"").slice(0,120)});
